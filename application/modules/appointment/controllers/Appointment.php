...
class Appointment extends MX_Controller
{
    ...
    function getAppointmentByJason()
    {
        ...
        foreach ($query as $entry) {
            ...
            if ($start_time[1] == 'AM') {
                $start_time_second = explode(':', $start_time[0]);
                if ((int)$start_time_second[0] == 12) {
                    $day_start_time_second = (int)$start_time_second[1] * 60;
                } else {
                ...
                $start_time_second = explode(':', $start_time[0]);
                if ((int)$start_time_second[0] == 12) {
                    $day_start_time_second = 12 * 60 * 60 + (int)$start_time_second[1] * 60;
                } else {
                    $day_start_time_second = 12 * 60 * 60 + (int)$start_time_second[0] * 60 * 60 + (int)$start_time_second[1] * 60;
                }
            }

            if ($end_time[1] == 'AM') {
                $end_time_second = explode(':', $end_time[0]);
                if ((int)$end_time_second[0] == 12) {
                    $day_end_time_second = (int)$end_time_second[1] * 60;
                } else {
                ...
                $end_time_second = explode(':', $end_time[0]);
                if ((int)$end_time_second[0] == 12) {
                    $day_end_time_second = 12 * 60 * 60 + (int)$end_time_second[1] * 60;
                } else {
                    $day_end_time_second = 12 * 60 * 60 + (int)$end_time_second[0] * 60 * 60 + (int)$end_time_second[1] * 60;
                }
            }
            ...
            $jsonevents[] = array(
                ...
                'start' => date('Y-m-d H:i:s', $entry->date + strval($day_start_time_second)),
                'end' => date('Y-m-d H:i:s', $entry->date + strval($day_end_time_second)),
                'color' => $color,
            );
        }
