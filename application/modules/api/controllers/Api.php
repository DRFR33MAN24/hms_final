...
function logToConsoleFile($data)
{
...
class Api extends MX_Controller
{
    ...
    public function authenticate()
    {
        ...
        log_message('error', "authenticate");
        $identity = $this->input->post('email');
        $password = $this->input->post('password');
        ...
        if ($mode == 'login') {
            ...
            if ($this->ion_auth->login($identity, $password, $remember)) {
                ...
                if ($group2->name == $group) {
                    ...
                    $data['error'] = 'error';
                    $data['message'] = 'login failed, check your email ';
                    echo json_encode($data);
                }
                // echo json_encode("b");
            } else {
            ...
            $id = $this->db->get_where('users', array( //making selection
                'email' => $identity
            ))->row();
            logToConsoleFile($id->id);
            if ($id->id) {
                $data['message'] = 'EMAIL_EXISTS';
                $data['error'] = 'error';
                echo json_encode($data);
            } else {
                ...
                $this->ion_auth->register($randomId, $password, $identity, $dfg);
                $id = $this->db->get_where('users', array( //making selection
                    'username' => $randomId
                ))->row();
                logToConsoleFile($id);
    ...
    {
        //get all the required filds from sent data
        ...
        if ($this->form_validation->run() == FALSE) {
            $data = 'failed';
            logToConsoleFile('form validation failed');
            echo json_encode($data);
        } else {
            $config = array(
                'file_name' => uniqid() . "_image",
                'upload_path' => "./uploads/",
                'allowed_types' => "*",
                'overwrite' => False,
        ...
        // send email to use to activate his/her account >
        // $autoemail = $this->email_model->getAutoEmailByType('patient');
        // if ($autoemail->status == 'Active') {
        //     $mail_provider = $this->settings_model->getSettings()->emailtype;
        //     $settngs_name = $this->settings_model->getSettings()->system_vendor;
        ...
        //     $this->email->send();
        // }
        //if ion contains some way to accomplish this use it
        // if not >
        //create new use and set active to false and set email hash to random value 
        // create validation route to check if user clicked the link sent to him.

    }
    function updatePatientProfile()
    {
        logToConsoleFile('updatePatientProfile');
        //get sent fields including image from sent data and update user record
        $id = $this->input->post('id');
        $name = $this->input->post('name');
        $password = $this->input->post('password');
        $email = $this->input->post('email');
    ...
    {
        logToConsoleFile('addAppointment');
        $id = $this->input->post('id');
        $patient = $this->input->post('patient');
        $doctor = $this->input->post('doctor');
        $date = $this->input->post('date');
        ...
        $this->hospitalID = $this->getHospitalID($ion_user_id);
        //check if patient in this hospital
        logToConsoleFile($patient);
        logToConsoleFile($user);
        logToConsoleFile($this->hospitalID);
        logToConsoleFile($ion_user_id);
        $this->db->where('patient_id', $this->api_model->getPatientByIonUserIdAllHospitals($patient)->id);
        ...
        if (!$result) {
            ...
            $data = array(
                ...
                'patient_id' => $this->api_model->getPatientByIonUserIdAllHospitals($patient)->id,
                'hospital_id' => $this->hospitalID
            );

            $this->db->insert('patients_hospitals', $data);
        }
        ...
        $patient_phone = $this->api_model->getPatientByIonUserIdAllHospitals($patient)->phone;
        $add_date = date('m/d/y');
        $registration_time = time();
        $temp_phone = str_replace('+', '', $patient_phone);
        ...
        $patient_details = $this->api_model->getPatientByIonUserIdAllHospitals($patient);
        $patientname = $this->api_model->getPatientByIonUserIdAllHospitals($patient)->name;
        $doctorname = $this->api_model->getDoctorById($doctor, $this->hospitalID)->name;
        $data = array();
        $data = array(
    ...
    {
        logToConsoleFile('getMyAllAppointmentList');
        $group = $this->input->post('group');
        $id = $this->input->post('id');
    ...
    {
        $patient_ion_id = $this->input->get('id');
        $hospital_id = $this->input->get('hospitalId');
        $this->hospitalID = $this->getHospitalID($patient_ion_id);
    ...
    public function getHospitalsList()
    {
        ...
        log_message('error', 'getHospitalsList');

        logToConsoleFile($region);

        $hospitals = $this->db->get_where('hospital', array('area' => $region))->result();
    ...
    {
        logToConsoleFile('getAllRegions');
        $sql = 'SELECT DISTINCT area FROM hospital;';

        $regions = $this->db->query($sql)->result();
        echo json_encode($regions);
        logToConsoleFile($regions);
    }
    public function getDoctorTimeSlop()
    {
        log_message('error', 'getDoctorTimeSlop');
        ...
        if (empty($date)) {
            ...
            $result = $this->getAvailableSlotByDoctorByDate($date, $id);
            echo json_encode($result);
        }
    }
    ...
    function getAvailableSlotByDoctorByDate($date, $doctor)
    {
        ...
        if (empty($holiday)) {
            ...
            foreach ($query1 as $timeslotkey => $timeslot) {

                $slot = $timeslot->s_time . ' To ' . $timeslot->e_time;
                foreach ($query as $bookedTimekey => $bookedTime) {
                    if ($slot == $bookedTime->time_slot) {

                        unset($query1[$timeslotkey]);
                    }
                }
            }

            usort($query1, function ($a, $b) {
                return $a->s_time_key > $b->s_time_key;
            });
    ...
    function getDoctorsByDepartmentname()
    {
        ...
        $data = $this->api_model->getDoctorByDepartmentname($department, $hospitalID);
        echo json_encode($data);
    }
    ...
    function getLabReportDetails()
    {
        ...
        $userId = $this->input->post('user_ion_id');
        // $group = $this->input->post('group');
        $data['labreport'] = $this->api_model->getLabByIdMM($labreportid);
        $this->hospitalID = $data['labreport']->hospital_id;
        if (!empty($data['labreport']->hospital_id)) {
            if ($data['labreport']->hospital_id != $this->hospitalID) {
                ...
                $data['user'] = $this->api_model->getPatientByIdMM($data['labreport']->patient);
                $data['message'] = 'successful';
